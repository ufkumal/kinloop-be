...
public class ChildAnswer {
    ...
    public static ChildAnswer ofOption(QuestionnaireSession session, Question question, QuestionOption option) {
        ChildAnswer answer = base(session, question);
    ...
    private static ChildAnswer base(QuestionnaireSession session, Question question) {
        ...
        answer.childId = session.getChildId();
        answer.question = question;
        answer.answeredAt = OffsetDateTime.now();
        return answer;
    }

    public void replaceWithOption(QuestionOption option) {
        this.option = option;
        setValueFromOptionCode(option.getCode());
        dateValue = null;
        answeredAt = OffsetDateTime.now();
    }

    private void setValueFromOptionCode(String optionCode) {
        try {
            numericValue = Integer.valueOf(optionCode);
            textValue = null;
        } catch (NumberFormatException ignored) {
            numericValue = null;
            textValue = optionCode;
        }
    }
}
